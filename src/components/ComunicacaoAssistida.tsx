    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div className="lg:col-span-5 space-y-6">
        ...
        <div className="tech-card">
          ...
          <div className="p-4 space-y-2 max-h-[220px] overflow-y-auto bg-white">
            {templates.map(tpl => (
              <button
                key={tpl.id}
                id={`tpl-btn-${tpl.id}`}
        ...
        <div className="tech-card">
          ...
          <div className="p-5 space-y-4 bg-white">
            <div id="placeholders-inputs" className="space-y-3.5 max-h-[320px] overflow-y-auto pr-1">
                ...
                <div key={ph.key} className="space-y-1">
                  ...
                  {ph.key === 'OBSERVACOES' || ph.key === 'MOTIVO' ? (
                    <textarea
                      ...
                      className="w-full px-3 py-1.5 bg-white border border-slate-250 rounded text-xs focus:outline-hidden focus:ring-1 focus:ring-indigo-500 text-[#111827] leading-relaxed font-sans"
                    />
                  ) : (
                    <input
                      type="text"
                      ...
                      onChange={(e) => handleFieldChange(ph.key, e.target.value)}
                      placeholder={ph.placeholder}
                      className="w-full px-3 py-1.5 bg-white border border-slate-250 rounded text-xs focus:outline-hidden focus:ring-1 focus:ring-indigo-500 text-[#111827] font-sans"
                    />
                  )}
                </div>
              ))}
      ...
      <div className="lg:col-span-7 flex flex-col space-y-6">
        ...
        <div className="tech-card flex-1 flex flex-col">
          ...
          <div className="p-6 bg-slate-50/30 flex-1 flex flex-col justify-between">
            
            {isAiLoading ? (
              <div className="flex flex-col items-center justify-center space-y-4 py-20 bg-white rounded border border-dashed border-slate-250 my-auto">
              ...
              <div
                id="compiled-preview"
                className="bg-white p-6 rounded shadow-2xs border border-slate-200 font-sans whitespace-pre-line text-xs sm:text-sm text-slate-850 leading-relaxed min-h-[250px] relative transition-all"
              >
                {compiledText}
              </div>
            )}
            ...
            {/* Actions panel */}
            <div className="flex flex-wrap items-center justify-between gap-3 mt-5 pt-4 border-t border-slate-150">
              
              <div className="flex items-center space-x-2">
                {isAiPolished && (
                  <button
                    onClick={handleReset}
                    className="flex items-center space-x-1.5 text-xs text-slate-500 hover:text-slate-850 py-1.5 px-3 border border-slate-200 rounded-full hover:bg-slate-50 cursor-pointer"
                  >
              ...
              <div className="flex items-center space-x-2">
                <button
                  ...
                  disabled={registered}
                  className={`flex items-center space-x-1.5 text-xs py-1.5 px-4 rounded-full border transition-colors cursor-pointer ${
                    registered 
                ...
                <button
                  id="action-copy-doc"
                  onClick={handleCopy}
                  className="tech-btn-primary flex items-center space-x-1.5 px-4 py-1.5 text-xs bg-[#111827] hover:bg-slate-800"
                >
                  {copied ? (
                    <>
        ...
        {AI_REFINEMENT_ENABLED ? (
          <div className="bg-[#111827] text-white p-6 rounded-lg border border-slate-900 shadow-xs flex flex-col md:flex-row items-stretch md:items-center justify-between gap-6">
            ...
            <button
              ...
              className="w-full md:w-auto flex-shrink-0 flex items-center justify-center space-x-2 bg-amber-400 hover:bg-amber-500 text-slate-900 px-5 py-3 rounded-full text-xs font-black uppercase tracking-wider transition-all cursor-pointer self-center md:self-auto"
            >
          ...
          <div className="bg-slate-100 text-slate-700 p-4 rounded-lg border border-slate-200 text-xs">
